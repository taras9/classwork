...
def group_group_collide(group1, group2):
    ...
    for group1_object in group1_copy :
        if group_collide(group2, group1_object):
            group1.remove(group1_object)        
    return len(group1_copy) - len(group1)
...
#    	self.radius = 
    ...
    def collide(self, other_object):
        ...
        radii_sum = other_obj_rad + self.radius
        dist_btwn_objs = dist(other_obj_pos, self.pos)
        if radii_sum >= dist_btwn_objs:
            return True
        if radii_sum < dist_btwn_objs:
            return False
...
def draw(canvas):
    global time, started, rock_group, score, lives
    ...
    my_ship.draw(canvas)
    a_rock.process_sprite_group(rock_group, canvas)
    a_missile.process_sprite_group(missile_group, canvas)
